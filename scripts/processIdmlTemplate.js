...
import {
  ...
  toIndoNum,
} from "../services/idmlDataProcessor.js";
...
export const processStoryUcc5Contextually = (xmlContent, rawData) => {
  ...
  const headers = [
    ...
    {
      header: "Perumahan, Air, Listrik, dan Bahan Bakar Rumah Tangga",
      key: "perumahan",
    },
    {
      header: "Perlengkapan, Peralatan, dan Pemeliharaan Rutin Rumah Tangga",
      key: "perlengkapan",
    },
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    const nextPos =
      i + 1 < matches.length ? matches[i + 1].pos : xmlContent.length;
    ...
    const subItems =
      Array.isArray(data.subYoy) && data.subYoy.length > 0
        ? data.subYoy
        : Array.isArray(data.subMom)
          ? data.subMom
          : [];
    let subHighest =
      subItems.length > 0
        ? subItems.reduce(
            (max, s) =>
              (parseFloat(s.value) || 0) > (parseFloat(max.value) || 0)
                ? s
                : max,
            subItems[0],
          )
    ...
    let subLowest =
      subItems.length > 0
        ? subItems.reduce(
            (min, s) =>
              (parseFloat(s.value) || 0) < (parseFloat(min.value) || 0)
                ? s
                : min,
            subItems[0],
          )
    ...
    let subStabilList = subItems.filter(
      (s) => (parseFloat(s.value) || 0) === 0,
    );

    // Dynamic inflasi vs deflasi wording
    if (yoyVal >= 0) {
      blockContent = blockContent.replace(
        /mengalami deflasi y-on-y/g,
      ...
      blockContent = blockContent.replace(
        /andil\/sumbangan deflasi y-on-y/g,
        "andil/sumbangan inflasi y-on-y",
      );
    } else {
      blockContent = blockContent.replace(
        /mengalami inflasi y-on-y/g,
        "mengalami deflasi y-on-y",
      );
      blockContent = blockContent.replace(
        /kenaikan indeks/g,
        "penurunan indeks",
      ...
      blockContent = blockContent.replace(
        /andil\/sumbangan inflasi y-on-y/g,
        "andil/sumbangan deflasi y-on-y",
      );
    }
    ...
    blockContent = blockContent.replace(
      /\$\{indeksTahunSebelumnya\}/g,
      toIndoNum(ihkPrevVal),
    );
    blockContent = blockContent.replace(
      /\$\{indeksSaatIni\}/g,
      toIndoNum(ihkNowVal),
    ...
    blockContent = blockContent.replace(
      /\$\{andilInflasiYoy\}/g,
      toIndoNum(Math.abs(andilYoyVal)),
    );
    blockContent = blockContent.replace(
      /\$\{andilDeflasiYoy\}/g,
      toIndoNum(Math.abs(andilYoyVal)),
    ...
    blockContent = blockContent.replace(
      /\$\{andilInflasiMtm\}/g,
      toIndoNum(Math.abs(andilMtmVal)),
    );
    blockContent = blockContent.replace(
      /\$\{andilDeflasiMtm\}/g,
      toIndoNum(Math.abs(andilMtmVal)),
    ...
    blockContent = blockContent.replace(
      /\$\{subkelompokInflasiTertinggi\}/g,
      subHighest ? subHighest.label.toLowerCase() : "makanan",
    );
    blockContent = blockContent.replace(
      /\$\{inflasiSubkelompokTertinggi\}/g,
      subHighest ? toIndoNum(Math.abs(subHighest.value)) : "0,00",
    );
    blockContent = blockContent.replace(
      /\$\{inflasiSubkelompokTertinggiYoy\}/g,
      subHighest ? toIndoNum(Math.abs(subHighest.value)) : "0,00",
    );

    blockContent = blockContent.replace(
      /\$\{subkelompokInflasiTerendah\}/g,
      subLowest ? subLowest.label.toLowerCase() : "minuman",
    );
    blockContent = blockContent.replace(
      /\$\{inflasiSubkelompokTerendah\}/g,
      subLowest ? toIndoNum(Math.abs(subLowest.value)) : "0,00",
    );
    blockContent = blockContent.replace(
      /\$\{inflasiSubkelompokTerendahYoy\}/g,
      subLowest ? toIndoNum(Math.abs(subLowest.value)) : "0,00",
    );

    blockContent = blockContent.replace(
      /\$\{subkelompokInflasi\}/g,
      subInflasiList.length > 0
        ? subInflasiList[0].label.toLowerCase()
        : "makanan",
    );
    blockContent = blockContent.replace(
      /\$\{inflasiSubkelompokYoy\}/g,
      subInflasiList.length > 0
        ? toIndoNum(Math.abs(subInflasiList[0].value))
        : "0,00",
    );

    blockContent = blockContent.replace(
      /\$\{subkelompokDeflasi\}/g,
      subDeflasiList.length > 0
        ? subDeflasiList[0].label.toLowerCase()
        : "pakaian",
    );
    blockContent = blockContent.replace(
      /\$\{deflasiSubkelompokYoy\}/g,
      subDeflasiList.length > 0
        ? toIndoNum(Math.abs(subDeflasiList[0].value))
        : "0,00",
    );

    blockContent = blockContent.replace(
      /\$\{subkelompokStabil\}/g,
      subStabilList.length > 0
        ? subStabilList.map((s) => s.label.toLowerCase()).join(", ")
        : "lainnya",
    );
    ...
    blockContent = blockContent.replace(
      /\$\{jumlahSubkelompokInflasi\}/g,
      String(subInflasiList.length),
    );
    blockContent = blockContent.replace(
      /\$\{jumlahSubkelompokStabil\}/g,
      String(subStabilList.length),
    );
...
export const runIdmlTemplateFiller = async (targetCity = "") => {
  console.log(
    `🚀 Memproses template IDML XML untuk kota: ${targetCity || "Default"}...`,
  );
  ...
  if (!fs.existsSync(IDML_DIR)) {
    throw new Error(
      `Direktori template idmlExtract tidak ditemukan di: ${IDML_DIR}`,
    );
  }
  ...
  const { variables: baseVars, raw: rawData } =
    await processIdmlVariables(targetCity);
  ...
  const idmlOutputPath = path.join(
    exportDir,
    `BRS_Populated_${cleanCity}.idml`,
  );
    ...
    for (const entry of entries) {
      ...
      if (stat.isDirectory()) {
        ...
        if (matches) {
          ...
          console.error(
            `❌ Ditemukan ${matches.length} placeholder belum terisi di: ${path.relative(OUT_DIR, fullPath)}`,
          );
  ...
  console.log(
    `- IDML Package Output: ${path.relative(process.cwd(), idmlOutputPath)}`,
  );
