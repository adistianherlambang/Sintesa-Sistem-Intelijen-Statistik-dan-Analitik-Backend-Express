...
// Ambil tanggal 1 bulan sebelumnya secara aman
...
export const fetchBI = async () => {
  try {
    ...
    for (let i in json) {
      ...
      const url = `https://www.bi.go.id/hargapangan/WebSite/TabelHarga/GetGridDataDaerah?price_type_id=2&comcat_id=&province_id=${prov}&regency_id=${kota}&market_id=&tipe_laporan=1&start_date=${startDateStr}&end_date=${endDateStr}&_=1784599800743`;
      ...
      while (!success && attempts < maxAttempts) {
        ...
        try {
          ...
          // Jeda 2 detik setelah fetch berhasil
          await new Promise((resolve) => setTimeout(resolve, 2000));
        } catch (err) {
          console.error(
            `Fetch gagal untuk kota ID ${kota} (percobaan ${attempts}/${maxAttempts}):`,
            err.message,
          );
