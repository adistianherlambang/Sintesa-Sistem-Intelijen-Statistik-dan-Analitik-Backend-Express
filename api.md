...
function UploadAnalysis() {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <input
        type="text"
        placeholder="Periode (ex: April 2026)"
...
function UpgradeSubscription() {
  ...
  return (
    <div>
      <button onClick={() => handlePay("premium_monthly")}>
        Beli Paket Bulanan (Rp 50.000)
      </button>
      ...
      {paymentInfo && (
        <div style={{ marginTop: 20 }}>
          ...
          <p>
            Total Nominal: Rp {paymentInfo.finalAmount.toLocaleString("id-ID")}
          </p>
...
const handleCheckStatus = async (invoiceId) => {
  try {
    ...
    if (transaction.status === "paid") {
      ...
      alert(
        message || "Pembayaran masih pending, silakan selesaikan pembayaran.",
      );
    }
  } catch (error) {
    console.error(
